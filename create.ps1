#####################################################
# HelloID-Conn-Prov-Target-SpeakApp-Create
#
# Version: 1.0.0
#####################################################
...
# Begin
try {
    Write-Verbose "Creating SpeakApp account for: '[$($p.DisplayName)]'"
    ...
    if ($dryRun -eq $true){
        $auditLogs.Add([PSCustomObject]@{
            Message = "$action SpeakApp account for: [$($p.DisplayName)], will be executed during enforcement"
        })
    }
    ...
    if (-not($dryRun -eq $true)){
        switch ($action) {
            ...
            'Correlate'{
                Write-Verbose "Correlating SpeakApp account for: [$($p.DisplayName)]"
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $errorMessage = "Could not $action SpeakApp account for: [$($p.DisplayName)]. Error: $($errorObj.ErrorMessage)"
    } else {
        $errorMessage = "Could not $action SpeakApp account for: [$($p.DisplayName)]. Error: $($ex.Exception.Message)"
    }
